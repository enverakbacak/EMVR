...
queryIndex = xlsread('Video_Dataset_3/qGroups_5d.xls');
filenames = filenames;
N = length(filenames);
%}
...
queryIndex5 = queryIndex(5,:); 
 ...
 for l = 1:100 % Number of Query Pairs , CAN TRY FOR DIFFERENT HAMMING RADIUS ALSO ?????? how?
   ...
   %%%%%%%%%%%%%%%%%%%%  ReRanking, rearrange P items by features  %%%%%%%%%%% 
    ...
    % Now remove last FIVE rows (qf1&qf2&qf3&qf4&qf5) from Retrieved items
    ...
    predicted_labels{l,:} = targets(Retrieved_Items_Ranked{l,:} , :);   
     ...
     % MQUR_Ranked_MQUR_scores{l,:} = MQUR_ALL{l,:}(MQUR_Ranked{l,:}, :);
     end
